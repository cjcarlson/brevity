...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 17 11:56:53 2019
...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan 16 17:10:37 2019
...
zoonotic network adding a node for humans
number of samples saved  for node size
calculate jaccard index and use that as the edge weight
...
Take Olival data and create unipartite host association networks for zoonotic viruses only
...
if __name__ == '__main__':
    ...
    zoonotic_host_association_net = add_edges_to_host_association_net(host_order_dict, unique_viruses, all_host_association_net, associations, is_virus_zoonotic_dict, order_virus_dict)
    zoonotic_host_association_net = get_num_samples_for_node_size(associations, host_order_dict, zoonotic_host_association_net, is_virus_zoonotic_dict)
    
    node_strength_dict = {}
    for each_node in nx.nodes(zoonotic_host_association_net):
        node_degree = zoonotic_host_association_net.degree(each_node)
        neighbor_weights = []
        node_neighbors = zoonotic_host_association_net.neighbors(each_node)
        for each_node_neighbor in node_neighbors:
            edge_weight = zoonotic_host_association_net[each_node][each_node_neighbor]['weight']
    ...
    for each_node in nx.nodes(zoonotic_host_association_net):
       ...
       each_node_strength = node_strength_dict[each_node]
       node_strength.append(each_node_strength)
    node_dict = {'Id':ids, 'Label': labels, 'Num_samples':num_samples, 'Node_strength':node_strength}
